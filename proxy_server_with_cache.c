...
//IN C THE MAIN PROBLEM IS OF TYPECASTING

typedef struct cache_element cache_element;


struct cache_element{       //linked-list
    char* data;         //data stores response
    int len;          //length of data i.e.. sizeof(data)... like in bites
    char* url;        //url stores the request...finds the url if 2nd time url is visited
...
cache_element* find(char* url); //find function in linked list
...
int handle_request(int clientSocket, ParsedRequest *request, char *tempReq) //returns if cache has the element OR fetch from server ad then stores in cache
{
...
int main(int argc, char * argv[]) {
	...
	bzero((char*)&server_addr, sizeof(server_addr));
    server_addr.sin_family = AF_INET;       //IPV-4
	...
	while(1)
	{
		...
		struct sockaddr_in* client_pt = (struct sockaddr_in*)&client_addr;  //basically making a copy
		struct in_addr ip_addr = client_pt->sin_addr;
		char str[INET_ADDRSTRLEN];										// INET_ADDRSTRLEN: Default ip address size
		inet_ntop( AF_INET, &ip_addr, str, INET_ADDRSTRLEN );           //inet_ntop -> Fetched ip address from network address after opening the socket
		printf("Client is connected with port number: %d and ip address: %s \n",ntohs(client_addr.sin_port), str); //ntohs -> to convert port
...
//make
//./proxy 8080
//on incognito -> http://localhost:8080/https://www.cs.princeton.edu/